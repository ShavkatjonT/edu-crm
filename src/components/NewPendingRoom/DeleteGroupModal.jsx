...
const DeleteGroupModal = ({columnData, closeFun, updateFun, loaderStart, loaderStop}) => {
    const { user } = useContext(Context);
    const navigate = useNavigate();
    const [deleteGroupName, setDeleteGroupName] = useState(columnData?.name);

    const columnsDelete = () => {
        loaderStart();
        http
            .post(import.meta.env.VITE_API_URL + 'api/columns/delete', {
                id: columnData?.id,
            })
            .then(() => {
                loaderStop();
    ...
    return (
        <AllGroupsDeleteModal>
            ...
            <div className="allGroups_delete_notification_part">
                ...
                <p>
                    Rostdan ham ushbu {deleteGroupName.length > 30 ? `${deleteGroupName.slice(0, 30)}...` : deleteGroupName} guruhini{' '}
                    o&apos;chirmoqchimisiz
                </p>
            </div>
