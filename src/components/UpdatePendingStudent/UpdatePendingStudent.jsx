...
function UpdatePendingStudent() {
  ...
  const studentAdd = () => {
    if (
      ...
      http
        .post(
          import.meta.env.VITE_API_URL + `api/new-pending-student/put/${data[4]}`,
          {
            firstname: firstname,
            lastname: lastname,
            gender: gender,
            birthday: birthday,
            fathername: fathername.length > 1 ? fathername : ' ',
            address: address,
        ...
        .then(() => {
          setSkeletonTime(false);
          navigate(`/pending-student/${data[4]}`);
        })
        .catch((error) => {
          if (
            error.response.data.message == 'ro\'yxattan o\'tmagan foydalanuvchi!'
      ...
      <Container>
        {
          ...
          <div>
            <i onClick={() => {navigate(`/pending-student/${data[4]}`);}} className='fa-solid exit fa-xmark'></i>
          </div>
        </Header>
        <hr />
        <Main>
...
export default UpdatePendingStudent;
